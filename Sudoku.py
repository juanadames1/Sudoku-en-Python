...
def number_in_row(grid, row, number):
    #Verificar sí el número se encuentra en la fila correcta
    return number in grid[row]
 
 
def number_in_col(grid, col, number):
    #Verificar sí el número está en la columna
    return number in (row[col] for row in grid)
 
 
def number_in_box(grid, row, col, number):
    #Verificar sí un número se encuentra en la caja que corresponde.
...
def reduce(n):
    #Función de reducción de posición de 9x9 a 3x3
...
def main():
    while True:
        ...
        # Verificar si la fuerza bruta dió resultado: si no hay más 
